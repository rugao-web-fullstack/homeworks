...
Mail.prototype.stateWrite = function (state, socket, data) {
  debug('log' + 'inside stateWrite');
  if (!state.action) {
    debug('log' + 'inside stateWrite home');
    this.stateWriteHome(state, socket, data);
  } else {
    debug('log' + 'inside stateWrite else ');
    switch (state.action) {
    case 'address':
    ...
    case 'body':
      ...
      break;
    case 'wait':
      debug('log' + 'inside stateWriteWait');
...
// 写邮件的页面
Mail.prototype.stateWriteHome = function (state, socket) {
  socket.write('\n请输入要发送的详细信息，\n\t1、收件人地址\n\t2、标题\n\t3、正文内容\n\t4、发送邮件\n');
  state.action = 'wait';
};
...
Mail.prototype.stateWriteWait = function (state, socket, data) {
  ...
  case '4':
    ...
    break;
  case 'exit':
    state.state = state.USER_LOGIN;
    state.action = '';
    socket.emit(states.USER_LOGIN, state, socket);
    break;
  default:
    debug('log' + 'insidestateWrite Default');
    break;
  }
};

// 写邮件时，接收地址输入的函数
Mail.prototype.stateWriteAddressWait = function (state, socket) {
  socket.write('请输入接收用户的地址：');
  state.action = 'address';
};

// 写邮件时，接收标题输入的函数
Mail.prototype.stateWriteTitleWait = function (state, socket) {
  socket.write('\n请输入标题：');
  state.action = 'title';
};

// 写邮件时，接收正文输入的函数
Mail.prototype.stateWriteBodyWait = function (state, socket) {
  socket.write('\n请输入正文内容：');
  state.action = 'body';
};
...
Mail.prototype.sendMail = function (state, socket) {
  let _this = this;
...
Mail.prototype.stateRead = function (state, socket, data) {
  debug('log' + 'inside stateRead');
  if (!state.action) {
    debug('log' + 'inside stateReadHome');
    this.stateReadHome(state, socket, data);
  } else {
    debug('log' + 'inside stateReadHome else');
    switch (state.action) {
    case 'wait':
...
Mail.prototype.stateReadHome = function (state, socket) {
  socket.write('\n++++++++++++++++++++++++++++++++++++++++');
  socket.write('\n请输入你要查看的邮件id: \n');
...
Mail.prototype.stateReadWait = function (state, socket, data) {
  debug('log' + 'inside stateReadWait');
