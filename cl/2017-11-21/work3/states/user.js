...
User.prototype.notLoginHome = function (machine, socket, data) {
  debug('info'+'inside user not login');
  debug('info'+data);
...
User.prototype.notLoginWait = function (machine, socket, data) {
  ...
  switch (input) {
  case '1':
    debug('info'+'inside not login wait 1');
    this.registerWait(machine, socket, data);
    break;
  case '2':
    debug('info'+'inside not login wait 2');
...
User.prototype.stateNotLogin = function (machine, socket, data) {
  debug('info'+'inside not login');
  if (!machine.action) {
    debug('info'+'inside not login home');
