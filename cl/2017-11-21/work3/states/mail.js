...
Mail.prototype.stateWrite = function (machine, socket, data) {
  debug('info'+'state write');
  if (!machine.action) {
    debug('info'+'state write home');
    this.stateWriteHome(machine, socket, data);
  } else {
    debug('info'+'inside not login else');
    ...
    case 'body':
      ...
      break;
    case 'wait':
      debug('info'+'inside not login wait');
...
Mail.prototype.stateWriteWait = function (machine, socket, data) {
  ...
  case 'exit':
    ...
    break;
  default:
    debug('info'+'inside not login wait default');
    break;
  }
};
/**
...
Mail.prototype.stateReadWait = function (machine, socket, data) {
  debug('info'+'state read wait');
  let index = machine.getCleanedString(socket, data);
  debug('info'+'input = ' + index);
  this.getMailList(socket, (error, mails) => {
    if (error) {
      socket.write('出错\n');
    }
    ...
    try {
      ...
      debug('info'+e.stack);
      debug('info'+'on error parse');
    }

  });
  // let mails = this.getMailList(socket);
  // debug("info"+"mail list:");
  // debug("info"+mails);
  // if (!mails) {
  //     return false;
  // }
